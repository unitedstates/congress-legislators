...
def run():
    ...
    def update_house_committee_metadata(xml_cx, cx, parentdict, is_subcommittee):
      ...
      if not is_subcommittee:
        for xml_sx in xml_cx.findall("subcommittee"):
          sxx = [
            s for s in cx["subcommittees"] if s["thomas_id"] == xml_sx.attrib["subcomcode"][2:]
          ]
    ...
    for xml_member in dom.xpath("/MemberData/members/member"):
      ...
      for cm in xml_member.xpath(
        ...
        if "comcode" in cm.attrib:
          ...
          thomas_committee_id = (
            house_ref[house_committee_id]["thomas_id"] + cm.attrib["subcomcode"][2:]
          )
        else:
  ...
  def scrape_senate():
    ...
    for id, name in re.findall(
      ...
      committee_url = (
        "https://www.senate.gov/general/committee_membership/committee_memberships_%s.htm" % id
      )
      ...
      committee_url = (
        "https://www.senate.gov/general/committee_membership/committee_memberships_%s.xml" % id
      )
  ...
  def scrape_senate_members(members, output_list, majority_party, is_joint):
    ...
    while i < len(output_list):
      if output_list[i]["bioguide"] not in ids and output_list[i].get("chamber") in (
        None,
        "senate",
      ):
        output_list[i : i + 1] = []
